    ...
    >
      {data?.map((item, index) => (
        ...
        >
          <div>
            <h2
              className={`${pathname.replace(/%20/g, "") === `/apphome/${workspace}/settings${item.url}`.replace(/( )+/g, "") ? " text-nyx-yellow underline decoration-4 underline-offset-8" : "text-white"} text-[20px]  font-normal`}
            >
              {item.name}
            </h2>
          </div>
            ...
            <div className="flex justify-center items-center">
              <div>
                {index === props.tabStatus ? null : (
                  ...
                  >
                    <g clip-path="url(#clip0_5839_11278)">
                      <path
